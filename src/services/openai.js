...
const INTENT_SCHEMA = {
  intent: {
    description: 'One of: greeting, discord_help, apply, provide_info, out_of_scope',
    enum: ['greeting', 'discord_help', 'apply', 'provide_info', 'out_of_scope'],
  },
  discord_help_requested: { type: 'boolean' },
};

const SYSTEM_PROMPT = `You are the recruitment assistant for ${STORE.name} (${STORE.url}), a platform that hires daily for online work-from-home jobs and pays in PKR. All communication with candidates happens on Discord.

YOUR JOB: Classify the candidate's message. Reply with ONLY a single JSON object, no markdown, no prose:

{
  "intent": "<greeting|discord_help|apply|provide_info|out_of_scope>",
  "discord_help_requested": true|false
}
...
- "discord_help" — the candidate says they do NOT have Discord, cannot access Discord, or asks how to install/set up Discord. Set discord_help_requested: true.
- "apply" — the candidate wants to apply for a job, start the application, register, join, or submit their name/phone/discord details. Also "haan main apply karna chahta hoon", "i want to join", "bharti karna hai".
...
// (knowledge/architecture.md), rendered from Untitled.pdf.
...
const GROUNDING_PROMPT = `You are the recruitment assistant for ${STORE.name} (${STORE.url}). You hire daily for online work-from-home jobs; all communication happens on Discord.
...
4. If the candidate names or asks about a specific job (e.g. "data entry", "video watch and earn"), reply with the INTENT_10 script verbatim (job selection + Discord transition).
...
6. Never ask for personal data yourself. If the user asks to apply or gives details, reply with exactly:
   APPLY_FLOW
7. If the user asks how to install Discord or says they don't have it, reply with exactly:
   DISCORD_HELP
...
async function classifyIntent(message) {
  try {
    ...
    const valid = ['greeting', 'discord_help', 'apply', 'provide_info', 'out_of_scope'];
    if (!valid.includes(parsed.intent)) {
      throw new Error(`Unknown intent: ${parsed.intent}`);
    }
    return {
      intent: parsed.intent,
      discordHelpRequested: parsed.discord_help_requested === true,
    };
  } catch (err) {
    ...
    return { intent: 'out_of_scope', discordHelpRequested: false };
  }
}
 ...
 * { discordHelp: true } sentinels handled by the conversation engine.
...
async function askGrounded(message, lang = 'en') {
  try {
    ...
    if (trimmed.toUpperCase().startsWith('DISCORD_HELP')) return { discordHelp: true };
    return { text: raw };
  } catch (err) {
